...
import type { CardRendererProps, IndicatorStatus } from '@/lib/indicators/types'
import { Sparkline } from '@/components/widgets/Sparkline'
import { formatNumber, formatPct, cn } from '@/lib/utils'

/**
 * Default indicator card — used by every indicator that doesn't ship its
 * own CardPlugin. Renders icon · name · date · value · change · sparkline
 * in the project's standard layout.
 */
export function DefaultCard({ snapshot }: CardRendererProps) {
  const { id, nameKr, unit, precision, value, previousValue, asOf, status, history } = snapshot
...
const statusConfig: Record<
  ...
  { color: string; Icon: typeof CircleDot; label: string }
> = {
